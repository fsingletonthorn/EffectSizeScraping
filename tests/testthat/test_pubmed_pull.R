...
expect_true(stringr::str_detect(output$text$text[2], stringr::fixed("which were significantly higher than those in cancer-adjacent normal tissues of the control group")))
            }
)
...
test_that("processPMC extracts correct values", {
  ...
  expect_equal(processedText$statisticalOutput$reported[1], "r=0.676, p<0.05")
