...
extractTestStats <- function(inputText, context = FALSE, contextSize = 100) {
  ...
  if(context == T) {
    ...
    extractedContext <- lapply(patternsContext, function(patternsContext)  
      unlist(str_extract_all(inputText, regex(as.character(patternsContext), ignore_case = T))))
 ...
 if(is_empty(statisticalOutput[[1]])) { 
   ...
   cbind(statisticalOutput, splitTestStatToDF(statistic = statisticalOutput$statistic, statisticalOutput$cleaned),stringsAsFactors = F)
  }
}
