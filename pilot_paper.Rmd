...
metadata <- data.table::fread("data_/extracted_data/metadata.csv")
stats <- data.table::fread("data_/extracted_data/statisticalTests.csv")
...
This data-table includes a total of `r nrow(stats)` reported statistics, including `r stats %>% filter(statistic %in% c("F", "t",  "chi")) %>% nrow` statistical test results and `r stats %>% filter(statistic %in% c("F", "t",  "chi")) %>% nrow`reported effect sizes from `r length(unique(stats$PMCID))` unique articles. In addition to the statistical test results (i.e., test statistics, degrees of freedom and p values) and effect sizes, each extracted result is reported with 100 characters of context extracted on either side of the detected result, and the label of the section that each result was extracted from (e.g., "Results" or "Methods").
...
Only one of these `r sum(!is.na(statistical_tests$statistic))` extracted statistical tests was found to be a false positive (i.e., `r (1/ sum(!is.na(statistical_tests$statistic))) * 100`% of checked results). 
...
Before checking the the results of the extraction efforts and blinded to the extracted test results, FST
